...
function buildPrompt(req: ReservationRequest): string {
  const langName = req.language === "ja" ? "Japanese" : "English";
  return `You are preparing a phone script for an AI agent that will call a restaurant in Japan and make a dinner reservation. The call will be conducted in ${langName}.
...
1. "phrases": every line the agent may need to say, in natural polite ${langName} (for Japanese: use appropriate keigo for a customer calling a restaurant; write numbers, dates and times the way they are SPOKEN, e.g. 「8月20日水曜日、19時に2名」). Cover at minimum: a greeting that states the purpose and the full reservation request in one sentence; answers for party size, date/time, name (include a spelled-out reading if the name could be misheard), and contact/phone-number follow-ups; a confirmation ("yes that's right"); a polite correction/denial; asking the other side to repeat ("すみません、もう一度お願いします"); a short hold phrase ("少々お待ちください"); handling "we're full" (ask about nearby times as a fallback, then thank and end); thanks; and a polite goodbye. Keep every phrase self-contained — it will be played as a standalone audio clip.

2. "expectedUtterances": everything the restaurant is likely to say, as intents with example phrasings and distinctive keywords (e.g. ask_party_size → 「何名様」「何名様でしょうか」, keywords 「何名」「人数」). Map each intent to the phrase ids that answer it. Include intents for: the restaurant answering the phone, asking party size / date / time / name / phone number, saying the slot is available, confirming the booking back (signalsSuccess), saying they are full (signalsFailure), asking the agent to wait, asking the agent to repeat, and closing the call (endsCall).
